###################################################################################
# LLM Model used: https://huggingface.co/mistralai/Mistral-7B-v0.1
# To download a model
# Step 1. Visit https://huggingface.co/mistralai/Mistral-7B-v0.1
# Step 2. git lfs install
# Step 3. git clone https://huggingface.co/mistralai/Mistral-7B-v0.1
...
# To provide access to single GPU
...
@serve.deployment(route_prefix="/mistral-7b", ray_actor_options={"num_gpus": 1}, num_replicas=1)
@serve.ingress(app)
class MistralDeployment:
    def __init__(self):
        ...
        self.max_memory_mapping = {0: "40GB"}
    ...
    def ask_query(self, request: LLMQuery):
        ...
        # this can be modified after seeing the output from the model.
        generated_answer = response_query.replace("</s>", "")
        # To extract only the relevant answer.
        generated_answer = generated_answer.strip(user_query)[-1]
...
deployment = MistralDeployment.bind()
